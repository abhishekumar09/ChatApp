...
export const signup = async (req, res) => {
  // export use because hmein signup ka route create krenge aur usmein rkhenge post request means usmein information provide krenge
  try {
    // try catch use krte hein ki error n aayee
    ...
    const hashedPassword = await bcrypt.hash(password, 10);
    const newUser = await new User({
      // Registring the new user means (signup)
      name,
      email,
      password: hashedPassword,
    });
...
// Define Api and fetching data (users)      jo ko getuser database se user ko get krega    // -password mtlb mujhe passwordd nhi chaiye
...
export const getUserProfile = async (req, res) => {
  try {
    ...
    console.log("Error is allUsers controller: " + error);
    res.status(500).json({ message: "Server error" });
  }
};
